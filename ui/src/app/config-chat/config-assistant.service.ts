...
import { ConfigChatContext } from './config-chat-context.service';

/** Transport for the Configuration chat. Parallel to the other chat
 *  transports but POSTs to `/api/v1/config-chat/chat` with the chat
 *  `sessionId` (the server scopes confirmation tokens by it) and an optional
 *  `{tab}` context. The endpoint shares the SSE event shape with the
 *  build-mode assistant — same parser, same AssistantEvent union — plus
 *  `confirm_request`. */
@Injectable({ providedIn: 'root' })
export class ConfigAssistantService {
  constructor(private auth: AuthService, private router: Router) {}

  chat(messages: ChatMessage[], context: ConfigChatContext | null, sessionId: string): Observable<AssistantEvent> {
    return new Observable<AssistantEvent>((observer: Observer<AssistantEvent>) => {
      ...
      const body: any = { messages, sessionId };
      if (context) body.context = context;

      fetch('/api/v1/config-chat/chat', {
        method: 'POST',
        credentials: 'include',
        headers,
