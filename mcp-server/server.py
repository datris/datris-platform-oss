...
Tap workflow (for step 3 Option B):
  ...
  3. If test fails: read the error, fix the script, and call create_tap again with a corrected script or updated instruction to regenerate. Repeat test until it succeeds.
     ...
     Size a large source BEFORE building: disk ≈ 26 bytes × column count × rows against `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB), time ≈ rows ÷ 15,000 per second when fetch() emits one record at a time or rows ÷ 90,000 per second when it emits batches, against `TAP_RUN_TIMEOUT_SECONDS` (default 3600 s) — show the user a rows, bytes/record, GB staged, minutes table and ask before creating anything; over either budget the choices are raise the variable, narrow the window, or chunk the range via run_tap params. Full rule in datris://tap-workflow-reference.
...
- Disk: records are staged as JSON with the keys repeated on every row, so bytes per record ≈ 26 bytes × column count for numeric / timestamp / short-string data (long text or nested fields add their own length). rows × that is the staged size, capped per run by `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB, 0 = unlimited).
- Time: a `fetch()` that emits one record at a time moves about 15,000 rows per second, one that emits batches about 90,000 rows per second — always name which rate the estimate used, and recommend batches for any source over about one million rows. Compare the minutes with `TAP_RUN_TIMEOUT_SECONDS`.
    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "Size a large source BEFORE creating anything: staged bytes per record ≈ 26 bytes × column count (JSON keys repeat on every row) against `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB, 0 = unlimited), and script time ≈ rows ÷ 15,000 per second emitting one record at a time or rows ÷ 90,000 per second emitting batches against `TAP_RUN_TIMEOUT_SECONDS` (default 3600 s) — name the rate you used, show the user a rows, bytes/record, GB staged, minutes table, and if either estimate is over its budget let the user choose first: raise the variable, narrow the window, or chunk the range via `run_tap` `params`. Reading only the columns the destination needs cuts both numbers linearly."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            ...
            description=(
                ...
                "SIZING RULE: before a first run on a large source, estimate staged disk as ≈ 26 bytes × column count per record × rows against `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB, 0 = unlimited), and script time as rows ÷ 15,000 per second when `fetch()` emits one record at a time or rows ÷ 90,000 per second when it emits batches against `TAP_RUN_TIMEOUT_SECONDS` (default 3600 s). Name the rate used, recommend batches above about a million rows, show the user the rows, bytes/record, GB staged, minutes table, and ask before running when either estimate is over budget.\n"
