...
Time-budget sanity check BEFORE building: a test is killed after `TAP_SCRIPT_TIMEOUT_SECONDS` (default 300 s) and a real or scheduled run after `TAP_RUN_TIMEOUT_SECONDS` (default 3600 s). The wrapper moves roughly 60k records per second, so an hour covers a few hundred million records. When the estimated record count would not finish inside the run ceiling, say so before building: the operator can raise `TAP_RUN_TIMEOUT_SECONDS`, or the range can be chunked via `run_tap` `params`. Never cap the rows to fit the clock.
    ...
    return [
        ...
        Tool(
            ...
            description=(
                ...
                "      - `test_mode`: ran in test mode (or mcp-server/datris version mismatch). Flag it; do not report data as stored.\n"
                "      - `run_error`: show the `error` string. If the error says the payload exceeded the disk budget (it names PIPELINE_MAX_PAYLOAD_MB), tell the user the operator can raise that variable; as a one-off alternative, reduce the source range via `params` and call run_tap again — multiple runs all land in the same destination pipeline. If the error says the run timed out (it names TAP_RUN_TIMEOUT_SECONDS), tell the user the operator can raise that variable; as a one-off alternative, chunk the source range via `params`. For a recurring tap, the durable fix is making the script incremental via DATRIS_TAP_STATE (see the tap-workflow-reference resource).\n"
                "VOLUME RULE: one run handles multi-GB payloads — records stream to disk, never through memory. Do NOT split a source into several runs, cap a run at N rows, or build resume-from-destination logic because you think the output is too big. Fetch the whole requested range in one run. Chunk only when the source API pages natively, when a single fetch would exceed the run timeout (`TAP_RUN_TIMEOUT_SECONDS`, default 3600 s), or when the user explicitly asks for a bounded window.\n"
