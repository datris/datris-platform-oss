...
The `get_lineage` MCP tool (or the neighborhood endpoint's `direction`, `depth` and `runs` parameters) lets an agent ask "what is downstream of this tap?" before changing or deleting it, or "what did this pipeline actually write lately?" before trusting a dataset. Direction is `up`, `down` or `both`; depth bounds the hops; runs appends the recent recorded runs.
