...
<Note>
**Test before you schedule.** A tap can only be saved with a schedule once the exact script (or HTTP endpoint) that will run on it has passed a test run. Saving a schedule on a never-tested or just-edited script is refused with HTTP 409. Remedy: save the tap without the schedule, run a test (Test Script in the UI, `test_tap` via MCP, or `datris tap test`), then set the schedule. Cadence changes on an already-tested tap need no re-test.
</Note>
