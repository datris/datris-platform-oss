...
- **Check the time budget too.** A test is killed after `TAP_SCRIPT_TIMEOUT_SECONDS` (default 300 s) and a real or scheduled run after `TAP_RUN_TIMEOUT_SECONDS` (default 3600 s). At roughly 60k records per second through the wrapper, an hour covers a few hundred million records — estimate the run up front, and if it will not fit, either raise `TAP_RUN_TIMEOUT_SECONDS` or chunk the range via `run_tap` `params`. A timed-out run says which mode it ran in and which variable to raise.
- **Check the payload budget before you build.** Records stream to the platform's staging disk, and one run may stage at most `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB). Estimate the run up front — rows × bytes per JSON record — and if the estimate exceeds the budget, decide before creating anything: raise `PIPELINE_MAX_PAYLOAD_MB`, choose a smaller feed or window, or chunk the range via `run_tap` `params`. Finding out after several minutes of streaming is the expensive way.
