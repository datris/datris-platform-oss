...
|---|---|
| `DATRIS_DATABASE` | The PostgreSQL database name (configured via `postgres.database` in `application.yaml`, or the tenant name in multi-tenant mode) |
...
The brainstorm AI knows about these endpoints, so if you mention "the X table on Datris" it will write the instruction to use them — no need to describe schema or column names; the script can introspect those at runtime via `/api/v1/metadata/postgres/columns`.
...
- Read `DATRIS_DATABASE`, `DATRIS_PLATFORM_HOST`, and `DATRIS_PLATFORM_PORT` directly without fallback defaults — they are always injected by the platform
