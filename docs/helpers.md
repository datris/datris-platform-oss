...
**Location:** `helpers/transformation-rest/`

A Flask REST API for external data transformation. The pipeline can call this endpoint as part of its transformation row functions, enabling custom transformation logic outside the pipeline. The sample implementation lowercases all string values. See [REST Endpoint Transformation](transformation/rest-endpoint.md) for the full protocol reference.
...
**Usage:** Configure a pipeline's `transformation.rowFunctions` with `function: "restEndpoint"` pointing to `http://host.docker.internal:5600/transform/rest/row` (row mode) or `/transform/rest/batch` (batch mode).
