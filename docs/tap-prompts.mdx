...
Fragments are managed via the `/api/v1/tap-prompts` REST endpoints (GET / POST / DELETE / `suggest`). The dedicated UI surface previously lived under **Configuration → Taps** and was removed when the Configuration tab list was simplified; the underlying data and API endpoints are unchanged, so existing fragments still apply to tap creation, brainstorm, auto-fix, and optimize exactly as before.
