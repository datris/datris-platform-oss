...
- **Single-tenant OSS with `USE_API_KEYS=true`**: append `--header x-api-key:<your-key>` to the `args` array. Manage entries from the Configuration UI's Secrets tab — each entry is `name=key`; the *value* is what your client sends.
