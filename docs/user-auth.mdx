...
- Pair this with `USE_API_KEYS=true` for any shared install — user auth gates the **UI**, API-key gating handles programmatic clients (CLI, MCP, external scripts). Setting `USE_API_KEYS=true` without `USE_USER_AUTH=true` is [refused at boot](/api-keys#supported-configurations); the two are designed to compose.
