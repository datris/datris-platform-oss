...
CODEGEN_MODEL=claude-opus-4-8   # or gpt-5 if using OpenAI
```
...
CodeGen tasks (tap scripts, `aiRule`, `aiTransformation`, JSON Schema / XSD generation, natural-language → SQL) have their own slot precisely so you can point them at a stronger model than the chatty default. With OpenAI the default is `gpt-5` for codegen vs `gpt-4.1` for general; with Anthropic the default is `claude-opus-4-8` for codegen vs `claude-sonnet-4-6` for general. Cheap chatty paths stay on the cheaper model so you only pay the upgrade on the calls that benefit.
