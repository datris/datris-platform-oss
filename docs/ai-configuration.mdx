...
You can run all three slots on a local Ollama instance — no cloud API keys needed. Ollama must be reachable from inside the Docker network. If Ollama runs on your host machine, use `host.docker.internal` as the hostname:

| Slot | Provider | Endpoint | Model (example) |
|------|----------|----------|-----------------|
| `oss/ai-primary` | ollama | `http://host.docker.internal:11434/v1/chat/completions` | `qwen3:14b` |
| `oss/codegen` | ollama | `http://host.docker.internal:11434/v1/chat/completions` | `qwen2.5-coder:7b-instruct` |
| `oss/embedding` | ollama | `http://ollama:11434/v1/embeddings` | `bge-m3` |

The embedding slot can use the bundled Ollama sidecar (`ollama:11434`) which auto-pulls `bge-m3`. For AI Primary and CodeGen, you must have the models available at your Ollama endpoint (e.g. `ollama pull qwen3:14b`).

The Configuration UI makes this easy — select **Ollama (local)** as the provider, type your model name, and the endpoint defaults to `host.docker.internal:11434`. No API key is required.
...
- **Ollama (bundled)** — `http://ollama:11434/v1/embeddings` (the bundled sidecar serves `bge-m3` out of the box)
- **Ollama (local)** — `http://host.docker.internal:11434/v1/embeddings` (your own Ollama with any embedding model)
