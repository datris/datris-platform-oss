...
### Onboard a new external data source

1. **Discover** — `discover_source` with a message like *"What datasets are available in yfinance?"* — the AI returns a structured list of datasets with parameters and `tapInstruction` for each
2. **Create taps** — call `create_tap` once per dataset you want, passing the `tapInstruction` as the instruction (Datris generates the Python script and tests it)
3. **Create pipelines** — `create_pipeline` for each tap with the destination of your choice
4. **Schedule** — `update_tap` with `cronExpression` and `enabled: true` to run on a recurring schedule
5. **Run** — `run_tap` to execute immediately, or wait for the schedule to fire
