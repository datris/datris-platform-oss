...
| `run_tap` | Execute a tap. Response carries `persisted` + `persistedReason` so the agent can confirm records landed, plus `publisherToken` + `pipelineTokens` for watching async loads |
...
| `get_pipeline_status` | Watch ingestion progress after `run_tap`. Pass the response's `publisherToken` (covers every job the run submitted — structured = 1, document = N) or a single `pipelineToken` |
...
6. **Run** — `run_tap`. Read the response's `persisted` field first:
   ...
   - `persisted: false` → read `persistedReason` (`no_target_pipeline`, `test_mode`, `run_error`, `no_records`), tell the user exactly why, and stop. Do NOT query the destination and report record counts from the response body.
7. **Verify it landed** — `get_pipeline_status(publisher_token=...)` and poll until every row's `state` is `end` or `error`. The tap log only tells you the script ran; the publisher token is how you confirm records actually reached the destination.
...
4. **Verify every document landed** — `get_pipeline_status(publisher_token=...)`. Document taps fan out to N jobs (one per file) under one publisher token; poll until all rows are `end` or `error`.
...
3. **Verify completion** — for any run of interest, call `get_pipeline_status(publisher_token=...)` to confirm records actually landed in the destination. The tap log tells you the script ran; the publisher token tells you the load finished.
4. **Diagnose failures** — on an errored row, read the `description` and either fix the tap (`create_tap` again with a corrected script) or retarget the pipeline (`update_tap`).
