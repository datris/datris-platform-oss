...
| `get_lineage` | Traverse the lineage graph from one node — upstream, downstream or both, to a bounded depth — with freshness and optionally the most recent recorded runs (what each read and wrote, per destination). Use it for impact analysis before changing or deleting something. See [Lineage](/provenance#lineage--freshness) |
