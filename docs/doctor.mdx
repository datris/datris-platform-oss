...
| `objectstore.bucket_overrides` | A pipeline overrides the built-in MinIO bucket and `DATRIS_OBJECTSTORE_BUCKET_ALLOWLIST` is unset (warn), or a stored override falls outside the list you set and will be refused at run time (error) | Set the allowlist, or add the bucket / change the pipeline's `destinationBucketOverride` |
...
| `staging.area` | The payload staging area (`DATRIS_TEMP_DIR`, the `datris-staging` volume) is missing, refuses a write, or has under 1 GB free (error); or its free space is below the per-run payload budget `PIPELINE_MAX_PAYLOAD_MB` (warn) | Point `DATRIS_TEMP_DIR` at a writable directory with room for one run, free space on the disk, or lower the budget. The 1 GB "room for one run" threshold is a floor, not a sizing guide: an upload can briefly occupy up to about three times its size (multipart spool, staged upload, the run's own copy) |
| `staging.orphans` | Run directories older than 24 h are still under the staging area — left by a process that died mid-run (warn; error when they hold ≥ 1 GB) | Restart datris (the boot sweep reclaims them) or remove the directories by hand |
...
Run it from the directory that holds `docker-compose.yml` and `.env`, or point it there with `--project-dir` (and `--compose-file` for the standalone file). Unlike the other CLI commands, `doctor` talks to the server directly rather than through the MCP server — a dead MCP server is itself a finding. Set `DATRIS_URL` if the server is not on `http://localhost:8080`. When API keys are on, set `DATRIS_API_KEY` — the same variable every other CLI command reads.
