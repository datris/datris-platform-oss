...
Agents can call the same query via the MCP `get_pipeline_status` tool — pass `publisher_token` from a `run_tap` response and poll until every job's latest row is `end` or `error`.
