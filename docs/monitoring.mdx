...
The [Datris UI](http://localhost:4200) provides a visual interface for managing your entire Datris platform. Five top-level tabs — **Assistant** (in-product agent), **MCP** (server status, tools, and a pop-out Agent Monitor for live AI agent activity), **Catalog** (taps and pipelines grouped by catalog), **Data** (ingestion uploads, job history, and semantic search across vector destinations), and **Configuration** (AI providers, secrets, API keys, users) — cover everything you can do through the API.
