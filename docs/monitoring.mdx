...
Agents call the same query via the MCP `get_pipeline_status` tool — pass `publisher_token` from a `run_tap` response and poll until `rollup.allDone` is `true`. The MCP tool sets `withrollup=true` automatically.
