...
The [Datris UI](http://localhost:4200) provides a visual interface for managing your entire Datris platform. It includes tabs for MCP server status and tools, pipeline management, ingestion monitoring with job history and error details, semantic search across vector databases, and secrets management — all without needing to use the API directly.
