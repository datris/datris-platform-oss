...
When `USE_API_KEYS=false` (the OSS default) no key is needed. When [`USE_API_KEYS=true`](/api-keys), export the key and the CLI sends it as the `x-api-key` header on every request to the MCP server:

```bash
export DATRIS_API_KEY=<your key>
```

If the key is missing or wrong, commands stop with a one-line message telling you to set `DATRIS_API_KEY`.

<Note>
Older CLI versions do not read `DATRIS_API_KEY`. For those, put the key on the MCP URL's query string instead, which the MCP server still accepts: `export MCP_SERVER_URL="http://localhost:3000/sse?api_key=<your key>"`.
</Note>
...
| `--dest, -d` | Destination: `postgres`, `mongodb`, `snowflake`, `databricks`, `qdrant`, `weaviate`, `milvus`, `chroma`, `pgvector` (default: `postgres`) |
| `--table, -t` | Table/collection name (default: pipeline name) |
| `--database` | Database name (default: `datris`). For `snowflake`: the Snowflake database; for `databricks`: the Unity Catalog name — required for both |
| `--schema` | Destination schema (`snowflake` default: `PUBLIC`; `databricks` default: `default`) |
| `--warehouse` | Snowflake warehouse name, or Databricks SQL warehouse ID (required for those destinations) |
| `--credentials-secret` | Platform secret holding destination credentials (required for `snowflake` and `databricks`) |
...
# Ingest into Snowflake (credentials come from a Platform secret)
datris ingest trades.csv --dest snowflake --database ANALYTICS \
  --warehouse COMPUTE_WH --credentials-secret snowflake-creds
...
| `--dest, -d` | Data source type: `postgres`, `mongodb`, `snowflake`, `databricks`, `qdrant`, `weaviate`, `milvus`, `chroma`, `pgvector` (default: `postgres`) |
| `--pipeline, -p` | Pipeline name (required for `snowflake`/`databricks` — the pipeline selects the connection) |
...
# Analyze a Snowflake or Databricks destination — pipeline selects the connection
datris analyze "Which region had the most sales?" --table sales --dest snowflake --pipeline sales
...
Unlike the other commands, `doctor` calls the server's REST API directly rather than going through the MCP server (`DATRIS_URL`, default `http://localhost:8080`). It reads the same `DATRIS_API_KEY` as every other command when API keys are on — a dead MCP server is itself a finding, not a reason to fail.
