...
Datasets are configured entirely through JSON. Each dataset defines a source, optional processing steps (preprocessing, data quality, transformation), and one or more destinations.
...
|-------|------|----------|-------------|
| `name` | string | Yes | Dataset name (max 80 characters) |
...
A single dataset can write to multiple destinations simultaneously. All destinations execute in parallel:
