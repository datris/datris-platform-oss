...
Nothing is landed: the run's rows are held briefly and handed back to the caller, then expire. Use it for an answer now — a validation result or a transformed view — never for data you want to keep. See [Scratch](/destinations/scratch).
...
`scratch` is the exception: a scratch pipeline lands nothing, so it cannot be combined with another destination.
