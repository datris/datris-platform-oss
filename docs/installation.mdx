...
1. **AI keys.** Anthropic (recommended for chat, CodeGen, AI data quality,
   ...
   optional and each is one Enter to skip. Using Azure OpenAI instead? Skip
   both prompts and the installer asks for your Azure key, resource
   endpoint, and chat deployment name next.
...
cp .env.example .env       # add ANTHROPIC_API_KEY and/or OPENAI_API_KEY (or the AZURE_OPENAI_* trio)
docker compose up -d
```
...
Datris supports four AI providers. Set your keys in `.env`:
