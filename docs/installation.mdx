...
- **The bundled `bge-m3` embedding model** in the `ollama-data` volume (will re-download ~1.2 GB on first start)
- **Cached pip wheels and tap-installed Python packages** in the `pip-cache` and `pip-packages` volumes (taps that need extras like `yfinance` will re-install on first run)
...
After `docker compose up -d`, `vault-init.sh` re-seeds the AI configuration secrets from your `.env`, MinIO buckets are recreated, Postgres starts empty, and Ollama re-downloads `bge-m3` (a few minutes one-time).
...
The `datris` service uses two named Docker volumes that are created automatically by `docker compose up` — no user action required:

| Volume | Mounted at | Purpose |
|---|---|---|
| `pip-cache` | `/root/.cache/pip` | Caches downloaded pip wheel files so tap-installed packages don't have to be re-downloaded |
| `pip-packages` | `/usr/local/lib/python3.12/dist-packages` | Persists installed Python packages across container restarts so taps that need extra packages (e.g., `yfinance`) install once and stay installed |

The first time a tap installs a new package, pip downloads it normally (~30 seconds for `yfinance`). Subsequent runs of the same or another tap that needs that package are near-instant — pip reports "Requirement already satisfied" and proceeds.
