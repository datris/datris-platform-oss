...
| Kafka | 9092 | Streaming — **optional, disabled by default** (uncomment in `docker-compose.yml` to run) |
| Zookeeper | 2181 | Kafka coordination — **optional, disabled by default** (uncomment with Kafka) |
| Kafka UI | 8085 | Kafka topic browser — **optional, disabled by default** (uncomment with Kafka) |
...
| **Kafka UI** (optional) | [http://localhost:8085](http://localhost:8085) | none — only when the optional Kafka services are uncommented in `docker-compose.yml` |
