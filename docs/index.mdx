...
**New here?** [Install in minutes](/installation) → [Quick Start](/quick-start) → [Connect your agent](/configuring-claude).
**Going to production?** [Security architecture](/production/security-architecture) → [Upgrades & supported versions](/production/upgrades) → [Licensing & support](/production/licensing-support).
**Building?** [Taps](/taps) → [Pipelines](/pipeline-configuration) → [Destinations](/destinations/postgres) → [API reference](/api-reference/pipeline-api).
...
- **Multiple destinations** - Write to PostgreSQL, MongoDB, Snowflake, Databricks, Kafka, ActiveMQ, REST endpoints, Qdrant, Weaviate, Milvus, Chroma, pgvector, or MinIO/S3 (Parquet, ORC, or Apache Iceberg tables) in parallel
...
| **Apache Spark** | Local Spark for writing Parquet, ORC, and Iceberg tables to MinIO |
