...
`docker/vault-init.sh` seeds all three secrets automatically based on which key is present in `.env` (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `OPENROUTER_API_KEY`). For multi-tenant deployments, per-tenant override secrets live at `{env}/ai-primary`, `{env}/codegen`, `{env}/embedding`.
...
|----------|---------|-------------|
| `AI_PROVIDER` | auto | Explicit provider pick (`anthropic`, `openai`, or `openrouter`) for the `ai-primary` and `codegen` slots. When unset, whichever API key is present wins (OpenAI wins ties; OpenRouter is chosen only when it's the sole key). |
...
| `OPENROUTER_MODEL` | `anthropic/claude-opus-5` | Model seeded into `oss/ai-primary` on the OpenRouter path (namespaced ids). |
| `CODEGEN_MODEL` | per provider | Model seeded into `oss/codegen`: `claude-opus-5` (Anthropic), `gpt-5.5` (OpenAI), `anthropic/claude-opus-5` (OpenRouter). |
| `EMBEDDING_PROVIDER` | follows provider | `openai`, `openrouter`, `tei`, or `ollama`. When unset: `openai` when the AI provider is OpenAI, `openrouter` when it's OpenRouter (hosted embeddings, no TEI download), otherwise the bundled TEI sidecar. |
| `EMBEDDING_MODEL` | provider default | `text-embedding-3-small` (openai), `openai/text-embedding-3-small` (openrouter), `BAAI/bge-m3` (tei), `bge-m3` (ollama). |
| `EMBEDDING_ENDPOINT` | provider default | Endpoint override for the `tei` and `ollama` providers — useful when TEI or Ollama runs on a separate host. |
