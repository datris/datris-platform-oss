...
| `AI_HTTP_MAX_PER_ROUTE` | `64` | Maximum concurrent connections the server's shared AI-provider and MCP clients open to one host. Every chat panel and the Assistant stream to the same provider host, so this is the limit concurrent chats hit first; when it is exhausted a call waits up to 10 seconds for a free connection and then fails with `Timeout waiting for connection from pool`. A blank, non-numeric or non-positive value uses the default. |
| `AI_HTTP_MAX_TOTAL` | `128` | Maximum concurrent connections across all hosts for the same clients. Never lower than `AI_HTTP_MAX_PER_ROUTE`; a smaller value is raised to match. A blank, non-numeric or non-positive value uses the default. |
