...
| `TAP_MAX_OUTPUT_MB` | unset | **Deprecated** alias of `PIPELINE_MAX_PAYLOAD_MB`, honoured for one release when `PIPELINE_MAX_PAYLOAD_MB` is unset. The bundled Compose files default `PIPELINE_MAX_PAYLOAD_MB`, so under Compose move your value to the new variable; the server logs a deprecation warning at boot while the alias is set. |
