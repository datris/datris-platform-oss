...
| `PIPELINE_MATERIALIZE_MAX_MB` | `256` | Largest staged payload (MB) a pipeline stage that still reads the whole payload into memory may materialize. Above it the run fails with an error naming the stage and this variable instead of exhausting the JVM heap. Raise together with `JAVA_OPTS` on larger hosts. |
