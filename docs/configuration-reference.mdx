...
| `pipelineMaxPayloadMB` | `4096` | Per-run payload disk budget in megabytes: the most a tap run may stage on disk (`DATRIS_TEMP_DIR`) before it is stopped with an actionable error naming `PIPELINE_MAX_PAYLOAD_MB` (the agent is told to raise it or chunk the source range via `run_tap` `params`). `0` = unlimited. Records stream to the staged file and never sit in JVM heap, so this bounds disk, not memory. Override per deployment with the `PIPELINE_MAX_PAYLOAD_MB` env var. |
