...
| `pipelineMaxPayloadMB` | `4096` | Per-run payload disk budget in megabytes: the most one run — a tap run, an upload or an object-store pickup — may stage on disk (`DATRIS_TEMP_DIR`) before it is stopped with an actionable error naming `PIPELINE_MAX_PAYLOAD_MB` (raise it or split the source; for a tap, chunk the source range via `run_tap` `params`). `0` = unlimited. Records stream to the staged file and never sit in JVM heap, so this bounds disk, not memory. Override per deployment with the `PIPELINE_MAX_PAYLOAD_MB` env var. |
