...
| `TAP_RUN_TIMEOUT_SECONDS` | `3600` | Override `tapRunTimeoutSeconds`: how long a real or scheduled tap **run** may take, in seconds. Raise it when a large streaming source needs more than an hour to land in one run; a timed-out run names this variable in its error. Unset resolves to `max(3600, tapScriptTimeoutSeconds)`. |
