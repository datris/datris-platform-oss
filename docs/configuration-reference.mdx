...
| `ai.aiPrimary.secretName` | `oss/ai-primary` | Vault secret for the main AI model used for general reasoning (NL→SQL, search answers, etc.). Seeded with the strongest available model — Anthropic gets `claude-opus-4-8`, OpenAI gets `gpt-5.5`. |
...
|----------|---------|-------------|
| `AI_PROVIDER` | auto | Explicit provider pick (`anthropic` or `openai`) for the `ai-primary` and `codegen` slots. When unset, whichever API key is present wins (OpenAI wins ties). |
| `ANTHROPIC_MODEL` | `claude-opus-4-8` | Model seeded into `oss/ai-primary` on the Anthropic path. |
| `OPENAI_MODEL` | `gpt-5.5` | Model seeded into `oss/ai-primary` on the OpenAI path. |
| `CODEGEN_MODEL` | `claude-opus-4-8` or `gpt-5.5` | Model seeded into `oss/codegen` (Anthropic and OpenAI defaults respectively). |
| `EMBEDDING_PROVIDER` | follows provider | `openai`, `tei`, or `ollama`. When unset: `openai` when the AI provider is OpenAI, otherwise the bundled TEI sidecar. |
| `EMBEDDING_MODEL` | provider default | `text-embedding-3-small` (openai), `BAAI/bge-m3` (tei), `bge-m3` (ollama). |
