...
| `scratchInlineRows` | `200` | For pipelines with a `scratch` destination: how many result rows ride inline on the run status (`resultPreview`), and the hard maximum page size for `GET /api/v1/pipeline/result`. Override per deployment with the `SCRATCH_INLINE_ROWS` env var. |
| `scratchRetentionHours` | `24` | How long a scratch result object is kept before the hourly sweep deletes it; reads after expiry return 410 and the caller runs the pipeline again. A scratch result holds whatever the source returned, so retention is the mitigation — keep it short. `0` expires results immediately. Override per deployment with the `SCRATCH_RETENTION_HOURS` env var. |
...
| `SCRATCH_INLINE_ROWS` | `200` | Override `scratchInlineRows`: rows inlined on a scratch run's status and the maximum `limit` per `/pipeline/result` page. |
| `SCRATCH_RETENTION_HOURS` | `24` | Override `scratchRetentionHours`: hours a scratch result object survives before the hourly sweep deletes it. Scratch results hold whatever the source returned, so shorten this rather than lengthen it on shared hosts. |
