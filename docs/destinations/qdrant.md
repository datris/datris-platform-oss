...
The embedding API is configured separately from the pipeline's AI provider, allowing you to use different services for embeddings vs. AI features.
...
Documents are split into chunks before embedding. Each chunk becomes a separate Qdrant point with the document's metadata plus `chunk_index`, `filename`, and `source_dataset` fields.
...
1. **Upload** — an unstructured file (PDF, text) is uploaded via `POST /api/v1/dataset/upload`
