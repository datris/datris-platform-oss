---
title: "Load data into Databricks Delta tables from an AI agent or pipeline"
sidebarTitle: "Databricks"
description: "Load validated data into Unity Catalog managed Delta tables through a SQL warehouse with COPY INTO, MERGE upserts, and per-pipeline credentials."
---

Use this destination when you want an agent or a scheduled pipeline to land validated data in Databricks without giving it workspace credentials directly. Each pipeline references its own secret, and every load is atomic, audited, and lineage-tracked.
