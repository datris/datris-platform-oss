---
title: "Embed documents into Qdrant for RAG"
sidebarTitle: "Qdrant"
description: "Chunk, embed, and upsert PDFs, text, and HTML into a Qdrant collection for retrieval-augmented generation."
---

Use this destination when you want a Qdrant index built and refreshed by a repeatable pipeline, with chunking and embedding handled for you.
...
Combine profiles with commas (e.g., `COMPOSE_PROFILES=qdrant,kafka`). The `install.sh` installer can also enable Qdrant at install time — it writes the profile and seeds the `oss/qdrant` secret for you. For the bundled container, use the compose service name as the host in the secret (`host="qdrant"`, gRPC port `6334`).
