...
For Anthropic-only deployments, the bundled Ollama sidecar serves `bge-m3` (1024-dim) and `vault-init.sh` seeds the embedding secret to point at it — no OpenAI key required.
