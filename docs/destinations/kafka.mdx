---
title: "Publish pipeline output to Kafka"
sidebarTitle: "Kafka"
description: "Write validated pipeline output to a Kafka topic with keyed partitioning, structured or raw payloads, and SASL/PLAIN auth."
---

Use this destination when downstream consumers read from Kafka and you want the messages validated before they are published.
...
Kafka credentials (`bootstrapServers`, `username`, `password`) are configured globally in `application.yaml` via `secrets.kafkaProducerSecretName`. See [Configuration Reference](/configuration-reference) for Vault secret format.
