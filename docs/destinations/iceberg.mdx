...
| `fileFormat` | yes | `parquet` | Set to `iceberg`. `parquet` and `orc` keep writing loose files |
...
| `deleteBeforeWrite` | no | `false` | Remove everything under the prefix before writing, which drops the table and recreates it |
...
- set **`deleteBeforeWrite: true`**, which removes everything under the prefix and creates the table fresh on the next run.
