...
To rotate or correct the keys, update the secret in the Secrets tab. The next run of any pipeline that uses that secret picks up the new keys; no restart is needed. A query against the same bucket that is already running at the moment the keys change can fail with "Filesystem closed" — re-run it.
