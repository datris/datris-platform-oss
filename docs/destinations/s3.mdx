---
title: "Write Parquet, ORC, or Apache Iceberg tables to AWS S3 from a pipeline"
sidebarTitle: "S3 (AWS)"
description: "Write validated pipeline output as Parquet files, ORC files, or an Apache Iceberg table to AWS S3 with per-bucket credentials and region."
---

Use this destination when your lake lives in S3 and you want validated, lineage-tracked files landed there by an agent or a scheduled pipeline, with credentials scoped per bucket.

The S3 destination writes Parquet files, ORC files, or an [Apache Iceberg table](/destinations/iceberg) directly to AWS S3. It uses the same Spark + S3A code path as the [MinIO destination](/destinations/object-store); the only differences are the credentials, the endpoint, and a few S3A settings applied per-bucket so multiple S3 destinations can coexist with the built-in MinIO output.
...
To rotate or correct the keys, update the secret in the Secrets tab. Fields you leave masked keep their current value, so you can change `region` or add `sessionToken` without re-entering `accessKey` and `secretKey`. The next run of any pipeline that uses that secret picks up the new keys; no restart is needed. A query against the same bucket that is already running at the moment the keys change can fail with "Filesystem closed" — re-run it.
...
These behave the same as the MinIO destination — see the [Object Store Destination](/destinations/object-store) page for the full reference, and the [Iceberg](/destinations/iceberg) page for table commits, `merge`, and schema evolution.
