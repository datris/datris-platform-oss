...
<Update label="v1.38.2" description="September 26, 2026">
  **Object-store ingestion is more forgiving: a bad file no longer blocks its neighbours, and root-level uploads work.**

  - **One bad file no longer stops the other files uploaded at the same time.** When several files land in the raw bucket together and one of them cannot be read, the others still run in the same pass. The failure is logged with its bucket and file name.
  ...
  - Upgrade: `docker compose pull && docker compose up -d --force-recreate`. Only the server image changed. See [Object-store ingestion](/ingestion/object-store).
</Update>

<Update label="v1.38.1" description="September 25, 2026">
...
<Update label="v1.38.0" description="September 25, 2026">
  **Change your platform's settings by asking: every change waits for your Confirm, and secrets never touch the chat.**
  ...
  - **Changes made through the assistant are marked in the audit log.** With user auth on, changes made through the assistant are attributed to the signed-in admin and marked `via: config-chat` in the audit log. See [Audit Log](/audit-log).
  - **More chats at once.** Several chats can stream at the same time without one failing on a connection-pool timeout; the limits are configurable, see [Configuration Reference](/configuration-reference).
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. Update all images together, including the UI. No configuration changes. With API keys and user auth on, the assistant is available in the UI only.
</Update>

<Update label="v1.37.1" description="September 24, 2026">
...
<Update label="v1.37.0" description="September 24, 2026">
  **Agents get live, validated reads from any source, and only land what's worth keeping.**
  ...
  - **The CLI signs in to the MCP server.** On installs with API keys turned on, every CLI command now sends the key you already export for `datris doctor`. A missing key gives a one-line message instead of a connection error, and a rejected key gives a one-line message and a non-zero exit instead of raw output and success. The query-string form still works. See [CLI](/cli).
...
<Update label="v1.35.0" description="September 21, 2026">
  **GPT-6 Astra support, and an Assistant that stays connected through long tool calls.**
  ...
  - **The Assistant no longer stops silently during long tool calls.** A tool call that ran for several minutes could end the conversation mid-turn with no message. The connection now stays alive for as long as the work runs, and if it is ever cut, the chat says so. The same applies to the Ops, Catalog and Search chats.
  - **Tap scripts stream their sources instead of downloading them.** The AI helpers, the Assistant and the documentation now state that a tap has only a small amount of scratch space and must read a remote file directly rather than save it first, and that a run's size should be estimated against the per-run disk budget before anything is built. See [Taps](/taps).
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. Update all images together, including the UI and the MCP server. No configuration changes required.
</Update>

<Update label="v1.34.1" description="September 20, 2026">
  **Taps stream large sources, and failures say what went wrong.**

  - **A tap can yield its records.** `fetch()` may return a generator or any iterator instead of a list; records are written to disk as they are produced, so a tap over millions of rows runs in a few tens of megabytes of memory. Scripts that return a list are unchanged. See [Taps](/taps).
  - **A tap killed for memory says so.** The error explains that the script was killed, almost certainly out of memory, and that `fetch()` should yield records instead of building the whole result. The Assistant and MCP guidance carry the same rule, stop to report after two failed tests, and no longer probe the runner environment.
  - **Rows returned as lists land in CSV pipelines.** A tap returning each row as a list with a header row now loads into a CSV pipeline with the header applied and names normalised, or fails with a message naming the problem, instead of corrupting the pipeline schema.
  - **A bad first line no longer changes the pipeline.** A CSV upload or object-store pickup whose header does not parse fails before schema evolution, leaving the schema and version untouched. Quoted header names resolve to their plain names.
  - **A blank schedule is no schedule.** Saving a tap with an empty schedule stores none and the scheduler stays quiet; taps already saved that way are quiet on upgrade.
  - **Testing a tap that yields stops at the test limit** instead of draining the source.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. Update all images together, including the MCP server. No configuration changes required.
</Update>

<Update label="v1.34.0" description="September 20, 2026">
  **A pipeline run is no longer limited by memory.**

  - **Payload size no longer limits a run.** Files uploaded over the API, archives uploaded as a batch, files picked up from the object store and records returned by taps all stream to disk as they arrive instead of being held in the server's memory. A multi-GB upload or tap run now completes on the default 2 GB heap, and memory use no longer grows with the size of the data passing through.
  - **The tap output cap becomes a per-run disk budget.** `PIPELINE_MAX_PAYLOAD_MB` (default 4096 MB, `0` = unlimited) is the most one run may stage on disk; a run that exceeds it stops with a message naming the variable and leaves nothing behind. Installs that never set the old `TAP_MAX_OUTPUT_MB` go from a 100 MB in-memory ceiling to 4 GB of disk. The old name is honoured for one more release, with a warning at startup. See [Taps](/taps).
  - **Uploads honour the same budget.** An uploaded file, or the combined contents of a batch archive, above the run budget fails with the same message. The upload ceiling stays at 1 GB by default and is now configurable with `DATRIS_MAX_UPLOAD_MB`.
  ...
  - **REST preprocessors and destinations can batch.** A `batchSize` sends the rows in batches of that size, each call carrying its batch number and the batch count; a REST row function in batch mode accepts a batch size as its sixth parameter. Without a batch size, both keep today's single call. See [Preprocessor](/preprocessor).
  ...
  - **`datris doctor` watches the staging area.** Two new checks report whether the staging area is writable with room for one run, and whether run directories left behind by a crash are accumulating. See [Doctor](/doctor).
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. Update the server and tap-runner images together; an older tap-runner still works but keeps its previous in-memory path. The compose file adds one named volume, `datris-staging`, for staged payloads — it is created on first start and needs no action. The bundled compose file now needs Docker Compose v2. No configuration changes are required; if `.env` sets `TAP_MAX_OUTPUT_MB`, rename it to `PIPELINE_MAX_PAYLOAD_MB` before the next release.
</Update>

<Update label="v1.33.1" description="September 18, 2026">
  **A schedule only ever runs code that has passed a test.**

  - **Test before you schedule.** A tap can carry a schedule only once the exact script that will run on it has passed a server-side test run. Saving a schedule on a never-tested or just-edited script is refused with a message naming the tap and the remedy: save unscheduled, test, then add the schedule. Cadence changes, clearing the schedule, and metadata edits on a tested tap need no re-test. See [Taps](/taps).
  - **Editing a scheduled tap never repoints the schedule at untested code** — by hand, by an agent over MCP, through the AI generate, fix, review and optimize helpers, or by restoring an earlier version. The scheduled script keeps running as it was until the new one is tested and the schedule re-applied.
  - **The wizard handles it for you.** Saving a scheduled tap saves it unscheduled, runs the test, and applies the schedule on success; on failure the tap is kept without its schedule and the wizard says so.
  - **Existing scheduled taps are untouched.** They keep running and stay editable; their first schedule or script change is accepted if the tap has ever tested or run successfully.
  - **Fix:** the CLI works on Python 3.14 (every command previously failed with an event-loop error).
  - **Fix:** upload, generate and profile return a plain error message instead of a stack trace.
  - **Fix:** the MCP Registry listing now names the settings the server actually reads; a Glama directory listing was added.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required. Agents editing a scheduled tap's script must clear the schedule first and re-test (the tools explain this on refusal). CLI users on pip or Homebrew: upgrade for the Python 3.14 fix.
</Update>

<Update label="v1.33.0" description="September 18, 2026">
  **Run a pipeline and get the rows back, without landing them.**

  - **Scratch destination.** For when you, or an agent, want an answer now rather than a dataset: what a source returns for these ids, whether today's file passes its rules, what a transformation produces. The run goes through the same preprocessing, data quality and transformation chain as any other pipeline; the rows are held briefly so the caller can read them back. Nothing is landed, catalogued or added to lineage, and the result expires after a retention window you control. Structured sources only; cannot be combined with another destination. See [Scratch](/destinations/live-read).
  - **Reading the result.** The first rows ride on the run's status. Larger results page from the run detail page's **View result** panel, from the CLI (`datris pipeline result`), and from the new `get_pipeline_result` MCP tool. Paging never re-runs the source.
  - **Agents know when to keep and when to scratch.** The MCP server and the Assistant follow a keep-or-scratch rule: data to be kept, observed, queried later or refreshed on a schedule goes to a real destination; a one-off answer goes to scratch. Fetch-only taps target a scratch pipeline; promote it later by changing the destination — the tap, its schedule and its incremental position do not move.
  - **Scratch in the UI.** The pipeline wizard offers Scratch under Destination; the Pipelines list badges scratch pipelines.
  - **Fix:** editing a secret keeps the values you did not change — saving one field no longer blanks the masked sensitive fields you left alone, in the UI and over the API.
  - **Fix:** the destination's authoritative flag set over the REST API was silently ignored; it is now applied and validated.
  - **Hardening.** S3 object-store endpoints pass the same network safeguards as other outbound connections; pipelines writing to the built-in object store can be restricted to an allowlist of buckets (off by default, reported by the doctor); query and status error responses no longer include internal diagnostic detail.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required. Scratch results are held for 24 hours by default; shorten it on shared hosts. CLI users on pip or Homebrew: upgrade to pick up `datris pipeline result`.
</Update>

<Update label="v1.32.0" description="September 17, 2026">
  **Pipelines can now land data as Apache Iceberg tables.**

  - **Iceberg as an output format.** Set the file format to `iceberg` on an [object-store](/destinations/object-store) or [S3](/destinations/s3) destination and each run commits a real Iceberg table at the pipeline's location: Parquet data files plus the table metadata any Iceberg-aware engine reads. One atomic commit per run; a failed run leaves the previous snapshot intact. No new destination type, no extra service. See [Iceberg](/destinations/iceberg).
  - **Upserts by key.** A new `merge` write mode updates rows matching the configured key fields and inserts the rest; re-running the same data leaves the row count unchanged.
  - **Partitioning and schema evolution.** Partition by columns; new nullable columns are added automatically, while type changes, dropped columns, and new required columns are refused before anything is written.
  - **Readable without Datris.** Standard Iceberg tooling opens the table by path. Time travel, catalog registration, table maintenance, and Iceberg as a tap source come later; tables written today will not need rewriting.
  - **Snapshot in query results** for Iceberg tables; the wizard, MCP tools, and Assistant know the new format and modes; lineage labels object-store destinations with their format.
  - **Fix:** querying an object-store pipeline that has never run returns an empty result instead of an error (a pipeline whose bucket does not exist also reads as empty).
  - **Overlapping runs** writing the same object-store location now run one after the other, for every object-store format.
  ...
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required; existing Parquet and ORC pipelines are unaffected. Switching an existing pipeline to Iceberg in place is refused unless `deleteBeforeWrite` is set; use a new prefix to keep the old files. The server image is about 50 MB larger.
</Update>

<Update label="v1.30.1" description="September 15, 2026">
  **A smaller install, and docs written for the way people search.**

  - **Minimal install.** The [installation guide](/installation) now leads with a minimal path that skips the bundled embedding server and bundled PostgreSQL and fits in a 4 GB Docker memory allocation; the full stack needs about 8 GB. Numbers measured on a live stack.
  - **OpenAI embeddings by default when you have a key.** Fresh installs with an OpenAI key present use OpenAI embeddings instead of the local embedding server, whichever provider handles chat. Existing installs are unchanged; set the embedding provider explicitly to keep embeddings local.
  - **New recipe:** [Give Claude Code a safe way to load data into PostgreSQL](/recipes/claude-code-postgres) — MCP connection, approval gates, a load, and the audit and lineage records it leaves.
  - **Docs pages named for the task.** Destination, MCP, policy, audit, lineage, and Airflow pages are titled for what you are trying to do. URLs unchanged.
  - **More reliable release builds** for the arm64 UI image.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required.
</Update>

<Update label="v1.30.0" description="September 14, 2026">
  **The doctor now runs itself.**

  - **Upgrades check themselves first.** Re-running the installer on an existing install runs the pre-upgrade self-check before pulling, when the `datris` CLI is on the machine. A warning continues; an error stops the upgrade with the fix on screen (and can be bypassed deliberately). Without the CLI the installer says so and continues as before.
  - **Continuous checks.** Set an interval and the server re-runs its self-checks on that cadence, posting to the incident webhook when a check flips to an error or recovers. Off by default; AI probes never run on the timer. See [Doctor](/doctor).
  - **The recovery agent can consult the doctor.** When an incident looks like the platform rather than the tap, the agent runs the quick self-check and hands the incident to a person with the doctor's fix.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required. **CLI users: upgrade to 1.29 or later** (`pip install -U datris-mcp-server` / `brew upgrade datris`) — the installer's pre-upgrade check needs it and will keep reminding you until you do.
</Update>

<Update label="v1.29.1" description="September 14, 2026">
  **Installs and upgrades work again.**

  - **MinIO images now come from quay.io.** MinIO removed its images from Docker Hub, so every fresh install and every `docker compose pull` failed with "pull access denied for minio/minio". The Compose files now pull the identical images from quay.io; running deployments were never affected. The installer, the standalone file and a fresh clone pick this up automatically.
  - **Docs: the production trail.** A new Production section walks a deployment from first install to a governed, audited, agent-operated setup; the Quick Start now ends on an approval and a provenance lookup.
  - Upgrade: `datris doctor --pre-upgrade`, then `docker compose pull && docker compose up -d --force-recreate`. Compose recreates `minio` on the new image name; the data volume re-attaches. No configuration changes required.
</Update>

<Update label="v1.29.0" description="September 10, 2026">
...
<Update label="v1.28.2" description="September 10, 2026">
...
<Update label="v1.28.1" description="September 8, 2026">
...
<Update label="v1.28.0" description="September 4, 2026">
...
<Update label="v1.26.1" description="September 2, 2026">
...
<Update label="v1.26.0" description="September 2, 2026">
...
<Update label="v1.25.0" description="September 1, 2026">
  **The recovery agent: Datris notices broken data flows, diagnoses them, and — within limits you set — repairs and verifies them.**
  ...
  - The [Agent Policy](/agent-policy) gains a **recovery mode**: `off`, `propose` (every repair waits for one-click approval), or `autopilot` (repairs follow your per-action policy) — with per-tap / per-pipeline overrides.
...
<Update label="v1.24.1" description="August 31, 2026">
...
<Update label="v1.24.0" description="August 31, 2026">
...
<Update label="v1.23.0" description="August 28, 2026">
...
<Update label="v1.22.0" description="August 27, 2026">
...
<Update label="v1.21.0" description="August 26, 2026">
...
<Update label="v1.20.2" description="August 25, 2026">
...
<Update label="v1.20.1" description="August 25, 2026">
...
<Update label="v1.19.3" description="August 21, 2026">
...
<Update label="v1.19.2" description="August 20, 2026">
...
<Update label="v1.19.1" description="August 20, 2026">
...
<Update label="v1.19.0" description="August 19, 2026">
  **Security hardening.**
...
<Update label="v1.17.0" description="August 17, 2026">
...
<Update label="v1.16.3" description="August 14, 2026">
...
<Update label="v1.16.2" description="August 13, 2026">
  **Azure OpenAI without API keys — Microsoft Entra ID authentication.**

  - **Keyless auth for Azure OpenAI.** Required for resources with `disableLocalAuth` (API keys disabled by policy), and useful anywhere you'd rather rotate credentials centrally. Chat, CodeGen, embeddings, and the Assistant all honor it. See [AI Configuration](/ai-configuration#keyless-auth-microsoft-entra-id).
