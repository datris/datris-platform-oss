...
<Update label="Unreleased" description="">
  **Change your platform's settings by asking: every change waits for your Confirm, and secrets never touch the chat.**
  ...
  - **Changes made through the assistant are marked in the audit log.** They are attributed to the signed-in admin, exactly as if made in the sub-tab, and carry `via: config-chat` in their detail. See [Audit Log](/audit-log).
