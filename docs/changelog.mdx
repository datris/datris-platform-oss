...
<Update label="v1.27.0" description="September 3, 2026">
  **Lineage now reflects what actually ran, not just what is configured — with an interactive graph and an agent tool to traverse it.**
  ...
  - Upgrade: `docker compose pull && docker compose up -d --force-recreate`. No configuration changes required. Runs are recorded from the first run after upgrading. CLI users on pip or Homebrew: upgrade to 1.27.0.
</Update>
