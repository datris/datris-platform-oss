...
| `tap` | A tap script calling back into the platform during a run | `crypto-prices` |
...
To record reads as well, set `auditLog.logReads: true` in `application.yaml`. Expect a lot more rows.
