...
`persisted: true` means the records were submitted to `targetPipeline`. When `persisted: false`, a `persistedReason` field names the cause — one of `test_mode`, `run_error`, `no_records`, `no_target_pipeline`. On persisted runs, `publisherToken` groups every ingestion job this run submitted; `pipelineTokens` lists each. Document taps fan out to many `pipelineTokens` but share one `publisherToken`.
...
```
GET /api/v1/pipeline/status?publishertoken={publisherToken}
```

Returns status rows for every ingestion job this tap run submitted (for structured taps that's one job; for document taps it's one per submitted document). Each row has a `state` — poll until every job's latest row is `end` (or `error`).
...
User-configured system prompt fragments that auto-inject into tap and discovery AI flows when the user's text matches the fragment's key or any alias. See [Tap Prompt Fragments](/tap-prompts) for the full concept; this section covers the REST endpoints.
