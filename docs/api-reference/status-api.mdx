...
**Live Read result fields.** A job whose pipeline is a [Live Read](/destinations/scratch) pipeline (the `scratch` destination) carries its result on the rollup once the run finishes. These fields are absent on every other job:
...
Page through the rows a [Live Read](/destinations/scratch) pipeline run produced. Paging reads the stored result — it never re-runs the source. Only Live Read pipelines have a result.
