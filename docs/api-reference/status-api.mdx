...
**Live Read result fields.** A job whose pipeline is a [Live Read](/destinations/live-read) pipeline (the `scratch` destination) carries its result on the rollup once the run finishes. These fields are absent on every other job:
...
| `resultTruncated` | boolean | `true` when the result has more rows than `resultPreview` holds — page the rest with [Get a Live Read Pipeline's Result](#get-a-live-read-pipelines-result). |
...
## Get a Live Read Pipeline's Result

```
GET /api/v1/pipeline/result?pipelinetoken={token}&offset={offset}&limit={limit}
```

Page through the rows a [Live Read](/destinations/live-read) pipeline run produced. Paging reads the stored result — it never re-runs the source. Only Live Read pipelines have a result.

**Parameters:**
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `pipelinetoken` | query | Yes* | Pipeline token of one ingestion job |
| `publishertoken` | query | Yes* | Publisher token from `/tap/run` — resolves to the Live Read job that run submitted |
...
| `404` | No run matched the token, or the pipeline is not a Live Read pipeline — only Live Read pipelines have a result |
