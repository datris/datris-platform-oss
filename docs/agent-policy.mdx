---
title: "Approval gates for AI agents writing to a database"
sidebarTitle: "Agent Policy"
description: "Decide per action whether an agent runs it unattended, waits for a person to approve it, or is refused. Enforced by the platform on every MCP client and API key, not by a prompt."
---
...
| The in-platform Assistant, Ops or Catalog chat, or the Configuration assistant, acting for a logged-in user | **Yes** — attributed to the user, but it is still the agent proposing |
