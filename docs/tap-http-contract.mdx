...
The request times out after the platform's tap timeout for the mode it was called in, the same
knobs as script taps: a **test** gets `TAP_SCRIPT_TIMEOUT_SECONDS` (default 300 seconds), a real or
scheduled **run** gets `TAP_RUN_TIMEOUT_SECONDS` (default 3600 seconds). Answer well within it —
see [Long fetches](#long-fetches-chunk-with-state).
...
The response is streamed to the platform's staging disk and capped at the per-run payload disk budget (`PIPELINE_MAX_PAYLOAD_MB`, default 4096 MB). Bigger
