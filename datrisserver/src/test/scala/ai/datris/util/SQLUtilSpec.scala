...
class SQLUtilSpec extends AnyFunSuite with MockitoSugar {
    ...
    test("maps common column types to their string values") {
        val rs = resultSetWith(
            List(("id", Types.INTEGER), ("name", Types.VARCHAR), ("big", Types.BIGINT), ("flag", Types.BOOLEAN), ("rate", Types.DOUBLE)),
            rs => {
        ...
        assert(rows == List(Map("id" -> "42", "name" -> "alpha", "big" -> "9999999999", "flag" -> "true", "rate" -> "1.5")))
    }
