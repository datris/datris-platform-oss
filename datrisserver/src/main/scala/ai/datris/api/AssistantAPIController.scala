...
class AssistantAPIController {
    ...
    private def buildSystemPrompt(workflowReference: String, tenantEnv: String): String = {
        ...
        sb.append("- **Structured / semi-structured taps** (CSV, JSON, XML, API responses, table-shaped data) → **MongoDB** by default (flexible schema, tolerates shape drift across runs). **PostgreSQL** and **object store** (columnar files — Parquet or ORC) are equally supported alternatives. When proposing a destination for structured data, briefly name all three options in the question (e.g. \"MongoDB by default; PostgreSQL or object store also available — which do you prefer?\") so the user can pick. Do not silently default to MongoDB without mentioning the alternatives — the user may not know the other options exist. **Snowflake** is also supported (a `database` destination with `useSnowflake=true`) — offer it when the user names Snowflake or already has a Snowflake `credentialsSecret` on the Platform tab; it needs that platform secret (account/user/key) before it can be used.\n")
