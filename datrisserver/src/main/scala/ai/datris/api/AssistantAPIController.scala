...
class AssistantAPIController {
    ...
    private def buildSystemPrompt(workflowReference: String, tenantEnv: String): String = {
        ...
        sb.append(
            "- **Never split a source for size.** One run handles multi-GB payloads — records stream to disk, never through memory. Do NOT cap a run at N rows, slice a month into several runs, or write resume-from-destination logic because the output looks large. Fetch the whole requested range in one run. Chunk only when the source API pages natively, when a single fetch would exceed the tap run timeout (TAP_RUN_TIMEOUT_SECONDS, default 3600 s), when the user explicitly asks for a bounded window, or when the payload-budget check below says one run cannot hold it. If a run fails saying the payload exceeded the disk budget, tell the user the operator can raise PIPELINE_MAX_PAYLOAD_MB.\n"
        )
        sb.append(
        ...
        sb.append(
            "- **Time-budget sanity check BEFORE building.** A tap test is killed after TAP_SCRIPT_TIMEOUT_SECONDS (default 300 s) and a real or scheduled run after TAP_RUN_TIMEOUT_SECONDS (default 3600 s). The wrapper moves roughly 60k records per second, so an hour covers a few hundred million records — divide the estimated record count by that rate and compare it with the run ceiling. If the run would not finish in time, say so before you create anything: the operator can raise TAP_RUN_TIMEOUT_SECONDS, or chunk the source range via `run_tap` `params`. Never cap the rows to fit the clock. A timed-out run's error names the mode it ran in and the variable to raise.\n"
        )
        sb.append(
        ...
        sb.append(
            "- **Stream the source, never download it.** The isolated runner has no disk of its own: its only writable area is a 512 MB in-memory scratch space shared with the per-run venv and every pip-installed package. A `fetch()` that saves a large file locally before reading it (`urlretrieve`, `open(path, \"wb\")`, `requests` `stream=True` written to a file, `tempfile`, libraries that spool under `TMPDIR`) fails with `No space left on device`. Read remote files directly and stream them — `pyarrow.parquet.ParquetFile(fsspec.open(url).open())` with `iter_batches()` (HTTP range reads; add `fsspec` to the packages), `pd.read_csv(url, chunksize=50000)`, `requests.get(url, stream=True).iter_lines()` — and `yield` records as they arrive. If a test fails with `No space left on device`, that is the diagnosis: fix the script this way, never ask for more scratch space first.\n"
        )
        sb.append(
