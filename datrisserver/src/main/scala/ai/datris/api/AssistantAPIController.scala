...
class AssistantAPIController {
    ...
    private def buildSystemPrompt(workflowReference: String, tenantEnv: String): String = {
        ...
        sb.append("- **Platform secrets (destination credentials) are read-only for agents.** Some destinations (e.g. `objectStore` with `provider=s3`) reference a `credentialsSecret` that holds destination credentials — NOT a tap secret. These live on the Platform tab and are owned by the user, not the agent. Use `list_platform_secrets` to discover what's available and `get_platform_secret_fields` to verify a candidate has the required keys (e.g. an S3 credentialsSecret must contain `accessKey`, `secretKey`, `region`). The agent CANNOT create, update, or delete platform secrets — if nothing suitable exists, tell the user plainly: \"You'll need to create a secret on the Configuration → Secrets → Platform tab with fields <list the keys>; then give me the name and I'll wire it into the pipeline.\" Do NOT try `create_tap_secret` for destination credentials — that would tag the secret `_type=tap` and the destination resolver would not find it.\n")
