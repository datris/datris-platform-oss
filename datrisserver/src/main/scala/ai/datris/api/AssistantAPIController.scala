...
class AssistantAPIController {
    ...
    private def buildSystemPrompt(workflowReference: String, tenantEnv: String): String = {
        ...
        sb.append(
            "- **Timeouts.** A tap test is killed after TAP_SCRIPT_TIMEOUT_SECONDS (default 300 s) and a real or scheduled run after TAP_RUN_TIMEOUT_SECONDS (default 3600 s). A test is also capped at 20 records, so a passing test says nothing about how long a real run takes — only the estimate above does. If the run would not finish in time, say so before you create anything: the operator can raise TAP_RUN_TIMEOUT_SECONDS, or chunk the source range via `run_tap` `params`. Never cap the rows to fit the clock. A timed-out run's error names the mode it ran in and the variable to raise.\n"
        )
        sb.append(
