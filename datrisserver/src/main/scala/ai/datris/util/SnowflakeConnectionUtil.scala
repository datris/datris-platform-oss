    ...
    def normalizePrivateKeyPem(raw: String, hasPassphrase: Boolean): String = {
        ...
        if (label == "RSA PRIVATE KEY")
            throw new DatrisException(
                "Snowflake 'privateKey' is a PKCS#1 key (BEGIN RSA PRIVATE KEY), which the Snowflake driver does not accept. Convert it to PKCS#8 first: openssl pkcs8 -topk8 -nocrypt -in <key-file>"
            )
        if (label == "ENCRYPTED PRIVATE KEY" && !hasPassphrase)
            throw new DatrisException(
                "Snowflake 'privateKey' is encrypted (BEGIN ENCRYPTED PRIVATE KEY) but the secret has no 'privateKeyPassphrase' field. Add the passphrase to the secret, or store an unencrypted PKCS#8 key"
            )
        ...
        if (body.isEmpty)
            throw new DatrisException(
                "Snowflake 'privateKey' has no key material after the PEM header — re-paste the full contents of the key file (the form field may have truncated it)"
            )
