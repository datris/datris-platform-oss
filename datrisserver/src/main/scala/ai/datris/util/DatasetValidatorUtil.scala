    ...
    private def validateStructuredAndSemiStructured(config: DatasetConfig): Unit = {
            ...
            if(config.dataQuality.columnRules != null) {
                ...
                config.dataQuality.columnRules.asScala.foreach(rule => {
                    ...
                    if(!schemaFieldNames.map(_.toLowerCase).contains(rule.columnName.toLowerCase))
                        throw new DatrisException("In the 'dataQuality.columnRules' section, columnName '" + rule.columnName + "' is not in the schema properties for this dataset")
        ...
        if(config.transformation != null) {
            if(config.source.fileAttributes != null && config.source.fileAttributes.csvAttributes == null)
                throw new DatrisException("A 'transformation' section is only supported for CSV files")
        ...
        if(config.destination.objectStore != null) {
            ...
            if(config.destination.objectStore.partitionBy != null) {
                config.destination.objectStore.partitionBy.forEach(field => {
                    if(!schemaFieldNames.contains(field))
                        throw new DatrisException("'partitionBy' field name: " + field + " is not in the schema properties for this dataset")
                })
            }
            ...
            val existingConfig = DatasetConfigIO.read(DatrisEnvironment.values.datasetTableName, config.name)
            if(existingConfig != null) {
                if(existingConfig.destination.objectStore != null) {
                    if(existingConfig.destination.objectStore.partitionBy != null && config.destination.objectStore.partitionBy == null)
                        throw new DatrisException("Cannot change an existing object store dataset from no partition to partitioned. Delete all S3 data for this dataset first and then re-register")
                    if(existingConfig.destination.objectStore.partitionBy == null && config.destination.objectStore.partitionBy != null)
                        throw new DatrisException("Cannot change an existing object store dataset from partitioned to not partitioned. Delete all S3 data for this dataset first and then re-register")
                }
            }
        }
        ...
        if(config.destination.database != null) {
            ...
            if(config.destination.database.keyFields != null) {
                config.destination.database.keyFields.forEach(field => {
                    if(!schemaFieldNames.contains(field))
                        throw new DatrisException("Key field: " + field + " is not in the schema properties for this dataset")
                })
            }
        ...
        if(config.destination.kafka != null) {
            ...
            if(config.destination.kafka.keyField != null) {
                if(!schemaFieldNames.map(_.toLowerCase).contains(config.destination.kafka.keyField.toLowerCase))
                    throw new DatrisException("destination.kafka.keyField '" + config.destination.kafka.keyField + "' is not in the schema properties for this dataset")
            }
        }
