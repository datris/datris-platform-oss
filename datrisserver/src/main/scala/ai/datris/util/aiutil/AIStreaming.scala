...
object AIStreaming {
    ...
    def callAIWithToolsStreaming(
        ...
        aiConfig.provider.toLowerCase match {
            ...
            case "openrouter" => openrouterNonStreamingCall(aiConfig, system, messages, tools, maxTokens, sink)
            case other =>
                throw new DatrisException("Provider '" + other + "' is not yet supported for chat. " +
                    "The chat assistants run on the AI Primary provider — set it to Anthropic, OpenAI, or OpenRouter in Configuration. " +
    ...
    // ---------- OpenRouter non-streaming fallback (chat/completions) ----------

    /** OpenRouter tool-use call. Non-streaming for v1 — synthesizes the same sink
      * events at the end so the UI rendering code stays provider-agnostic.
      * OpenRouter only speaks chat/completions, never the OpenAI Responses API,
      * regardless of the underlying model (ids like openai/gpt-5-codex included). */
    private def openrouterNonStreamingCall(
        aiConfig: AIConfig,
        system: String,
        messages: Seq[(String, List[AIContentBlock])],
        tools: List[JsonObject],
        ...
        logger.info("Assistant: OpenRouter chat/completions call, model=" + aiConfig.model + ", tools=" + tools.size +
