...
- **Faster repeated queries.** Postgres connections are now pooled and reused across pipeline loads and interactive queries instead of being opened fresh each time — noticeably snappier when agents or dashboards issue many queries in a row.
- **Connection details are masked in logs.** Database connection information no longer appears in logs or job status output.
- **More stable long-running deployments.** Fixed several resource leaks in file ingestion and data-reading paths that could slowly accumulate open connections over time.
- **Clearer diagnostics.** Many background failures that were previously silent are now logged with context, making troubleshooting faster.
- **Every release is now test-gated.** Datris has an automated test suite that runs on every change and before every release build, so regressions are caught before they ship.
