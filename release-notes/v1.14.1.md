...
v1.14.0 introduced a per-tap sync-state view — the current bookmark, when it was committed, and a one-click reset for a full re-fetch — but the button to open it was missing from the tap list, so the view couldn't be reached in the UI. Each tap row on the Catalog page now has a bookmark icon that opens it. Reading and resetting sync state through the MCP tools was unaffected.
